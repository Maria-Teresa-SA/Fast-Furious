...
@dataclass
class Station:
    """Infpormació associada a un node del graf de tipus Estacions
    de metro."""
...
@dataclass
class Access:
    """Informació associada a un node del graf de tipus Accessos a
...
def read_stations() -> Stations:
    ...
    for i, row in data_table_stations.iterrows():
        p: List[str] = row[cols[2]].strip('POINT( )').split()
        s = Station(row[cols[0]], row[cols[1]], Coord(float(p[0]), float(p[1])), '#' + row[cols[3]])
        stations.append(s)
...
def read_accesses() -> Accesses:
    ...
    for i, row in data_table_accesses.iterrows():
        p: List[str] = row[cols[3]].strip('POINT ( )').split()
        a = Access(row[cols[0]], row[cols[1]], Coord(float(p[0]), float(p[1])), 'black')
        accesses.append(a)
...
def get_metro_graph() -> MetroGraph:
    ...
    """Genera el graf amb nodes Estacions i Accessos als metros i les arestes requerides
    entre aquests.
    A cada node li és assignat un identificador i atributs que n'especifiquen
    el tipus i tota la informació necessària (ex. posició).
    Les arestes també tenen un atribut amb la informació del seu tipus, temps que
    es triga en ser recorregudes i el seu color.
    """
    ...
    # diccionari que ens connectarà amb un Enllaç els nodes
    # estació que comparteixen parada però no línia.
